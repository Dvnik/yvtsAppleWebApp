...
class myMark:NSObject, MKAnnotation{ //因為只有NSObject的後代才有資格再繼承某個介面
    //所以先讓myMark繼承 NSSObject <=== 這個類別是整個swift物件導向類別樹的最上層的祖先
    
    var coordinate: CLLocationCoordinate2D  // 一定要override MKAnnotation 的CLLocationCoordinate2D資料
    var title: String?                      // 一定要override MKAnnotation 的title(String)資料
    var subtitle: String?                   // 要不要override MKAnnotation 的subtitle(String)資料 無所謂
    //寫個建構子來把東西初始一下
...
//            annotationView.image = UIImage(named: "dog1")
