...
class member: UIViewController
{
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        print("b 沒事不要override寫我，如果發動segue前後有事想做就寫我")
        
        if segue.identifier == "go_to_vip" {
            
        }
        else if segue.identifier == "go_to_welcome" {
            let desitny_controller:food = segue.destination as! food
            desitny_controller.member_name = self.account_txt.text!
        }
