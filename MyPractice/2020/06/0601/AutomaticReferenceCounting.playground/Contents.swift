...
無論你將『實體』設定給屬性、常數或變數，它們都會建立此『實體』的"強引用"（strong reference）。也就是把「引用計數」加一！
...
class Person
{
    ...
    //反初始化方法
    /*
    『反初始化方法』在一個類別的實體被釋放之前，會立刻被呼叫。
    以關鍵字deinit來標示『反初始化方法』，類似於『初始化方法』以init來標示。
    『反初始化方法』只適用於類別！

    Swift利用"自動引用計數"（ARC）來處理實體的記憶體管理。
    通常當你的實體被釋放時，不需要手動去清理。
    但是，當使用自己的資源時，你可能需要進行一些額外的清理。
    例如：
    如果建立一個自定類別來打開一個文件，並寫入一些資料，你可能需要在類別實體被釋放之前，關閉該文件。

    『反初始化方法』不帶任何參數，在寫法上不帶括號：
    deinit
    {
        // 執行反初始化程序
    }

    『反初始化方法』是在實體釋放發生的前一步"自動被呼叫"，不允許主動去呼叫自己的『反初始化方法』。
    子類別繼承了父類別的『反初始化方法』，並且在子類別『反初始化方法』實作的最後，父類別的『反初始化方法』自動被呼叫。
    即使子類別沒有提供自己的『反初始化方法』，父類別的『反初始化方法』也總是被呼叫。

    因為直到實體的『反初始化方法』被呼叫時，實體才會被釋放，所以『反初始化方法』可以存取所有請求實體的屬性，並且根據那些屬性可以修改它的行為（例如：查找一個需要被關閉的文件名稱）。
    */
    deinit
    {
        //釋放時印出資訊
...
reference2 = reference1     // 此時引用計數為2(即引用計數加1)
reference3 = reference1     //現在這個Person實體已經有三個"強引用"了，引用計數為3

//如果你把兩個變數設定成nil來斷開兩個"強引用"（包括最先的那個"強引用"），只留下一個"強引用"，Person實體還不會被釋放：
reference1 = nil            // 此時引用技術由3降為2(即引用計數減1)
reference2 = nil            // 此時引用技術由2降為1(即引用計數減1)

//ARC會在第三個，也就是最後一個"強引用"被斷開的時候，釋放掉Person實體：
reference3 = nil            // 此時引用技術由1降為0(即引用計數減1)，同時回收記憶體配置空間
...
john = Person1(name: "John Appleseed")       // 此時Person1的實體引用計數為1
unit4A = Apartment1(unit: "4A")              // 此時Apartment1的實體引用計數為1

// 參閱<圖一>


//接著，將這兩個實體關聯在一起，這樣人就有公寓住，而公寓也有了房客：
john!.apartment = unit4A // 實務上是這樣做 >> john?.apartment = unit4A
// 注意：此時Apartment1的實體引用計數增為2，Person1的實體引用計數維持為1

unit4A!.tenant = john    // 實務上是這樣做 >> unit4A?.tenant = john
// 注意：此時Person1的實體引用計數增為2，Apartment1的實體引用計數維持為2
...
// 注意：此時Person1實體的飲用技術只將為1，ARC不會釋放記憶體！
unit4A = nil    //想釋放unit4A時，john之內有個屬性來抓住unit4A
// 注意：此時Apartment1實體的飲用技術只將為1，ARC不會釋放記憶體！
...
//---------------------弱引用（Weak References）---------------------
/*
『弱引用』不會牢牢保持住引用的實體，也不會阻止ARC釋放被引用的實體，這種行為避免了引用變成『循環強引用』。
...
你可以像檢查其他"選擇值"一樣，去檢查『弱引用』的值是否存在，你永遠不會引用到一個被釋放而不存在的實體（a reference to an invalid instance that no longer exists）。
 ...
 注意：
 當ARC將一個弱引用設為nil時，屬性觀察器（Property Observers）不會被呼叫！
 PS.屬性觀察器即屬性的觸發機制willSet或didSet
 */
 ...
 john2!.apartment = unit4A2      //『強引用』此時Apartement2的引用計數為2
 unit4A2!.tenant = john2         //這裡是『弱引用』此時Person2的飲用技術仍維持在1(因為弱引用不會增加引用計數)
...
注意:
如果你試圖在實體被釋放之後，存取該實體的『未持有引用』，會觸發執行時期錯誤（runtime error）。
使用『未持有引用』時，你必須確保引用始終指向一個未釋放的實體。
*/

/*
下面的範例定義了兩個類別：Customer和CreditCard，模擬了『銀行客戶』和『客戶的信用卡』。
這兩個類別中，每一個都將另外一個類別的實體作為自身的屬性。這種關係會潛在地創造『循環強引用』。
...
<場景2>Customer和CreditCard的範例展示了一個屬性的值允許為nil（註：客戶不一定有信用卡），而另一個屬性的值不允許為nil（註：信用卡一定要關聯一位客戶），並會潛在的產生『循環強引用』。這種場景最適合透過『未持有引用』來解決。
＃註：Ａ不一定要擁有Ｂ，可是一旦擁有了Ｂ，Ｂ就是Ａ的了，譬如：人不一定要養貓，但養了貓之後，貓就是那個主人的了＃
...
class Country
{
    ...
    init(name: String, capitalName: String)
    {
        ...
        //注意：這個self要在所有儲存屬性都有初值時即產生，但此時capitalCity仍為nil
        //Step3.此時已完成<第一階段>的初始化過程，已經產生實體self，將執行第二階段進一步更動屬性值的動作
        self.capitalCity = City(name: capitalName, country: self)
    }
    deinit {
        print("Country:\(name)被釋放！")
    }
}
...
{
    let name: String
    unowned let country: Country    //『未持有引用屬性』一個城市必須"屬於"一個國家
...
當你把一個"閉包"設定給某個屬性時，你也把一個引用設定給了這個"閉包"。
...
let defaultText = "這是預設文字" // <h1>這是預設文字</h1> or <h1 />
...
// heading.text != nil ? heading.text! : defaultText

//使用範例2：<p>
var paragraph: HTMLElement? = HTMLElement(name: "p", text: "hello, world") // <p>hello, world</p>
...
paragraph1?.asHTML =
{      //重新定義的閉包當沒有提供文字時，不會使用nil，一律改成『這是預設文字』字樣
    [unowned paragraph1] in
    return "<\(paragraph1!.name)>\(paragraph1!.text ?? defaultText)</\(paragraph1!.name)>"
}
