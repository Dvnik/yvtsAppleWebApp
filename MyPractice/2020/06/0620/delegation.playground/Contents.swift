...
//如下所示，DiceGameTracker類別採納了DiceGameDelegate協定，實作了代表遊戲狀態的三種協定方法：
//遊戲追蹤類別（註：此類別的實體就是"遊戲代理人"，代理人必須實作協定方法）
class DiceGameTracker: DiceGameDelegate     //注意：這是"實作"代理機制的類別！（通常是自己撰寫的類別，或某一個"控制器類別"）
{
    //記錄遊戲已經進行的輪數（儲存屬性）
    var numberOfTurns = 0
    

    //實作『遊戲開始』的協定方法，傳出遊戲的狀態訊息
    func gameDidStart(game: DiceGame)
    {
        numberOfTurns = 0
        //檢測傳入的game實體是否為SnakesAndLadders的類別實體
        if game is SnakesAndLadders
        {
            print("蛇與梯子的新遊戲開始！")
        }
        print("遊戲使用一顆\(game.dice.sides)面的骰子")
    }
    ...
    func gameDidEnd(game: DiceGame)
    {
        print("遊戲經過了\(numberOfTurns)輪")
    }
}

//注意：以下的運作，在實際的App中，通常由某一個"控制器類別"來執行！
//產生遊戲追蹤類別之實體
