...
/*
 ● Main佇列～
 每個應用程式只有一個Main佇列，也就是預設佇列。Main佇列只會在"同一個執行緒"中（thread），按照先進先出（FIFO）的方式依序執行。
 此外，Main佇列還需要在使用者觸發UI元件上的各種事件時，呼叫對應的處理函式，因此Main佇列只能透過async()非同步函式，將程式碼區段放到佇列中。
 
 非同步  asynchronous ： 可以同時執行
 
 同步    synchronous ：不可以同時執行
*/
...
/*
 ...
 Globle佇列又稱Concurrent佇列，每個App都有兩種不同優先權的Globle佇列，分別為userInteractive、userInitiated、utility、預設優先權、背景優先權。
...
/*
 以下兩種情境是GCD最典型的用法：
 1.如果你想要使用GCD進行非UI相關的任務，在不是Main佇列的佇列上使用sync()函式或async()函式。
