...
class MyTableViewController: UITableViewController, XMLParserDelegate
{
    ...
    //--------------------MySQL增加-------------------
    // 記錄主要網域名稱（網址開頭的部分）
    let webDomain = "http://172.16.100.191/MyDataBase/"
...
//                    self.arrTable.removeAll()
                    ...
                    // 指定XML解析過程的代理事件時坐在此類別
                    xmlParser.delegate = self
                    // 開始解析XML文件(此時會開始觸發XMLParserDelegate)
                    xmlParser.parse()
                }
...
//        }
    ...
    func parser(_ parser: XMLParser, didEndElement elementName: String, namespaceURI: String?, qualifiedName qName: String?) {
        ...
        switch elementName
        {
            ...
            case "gender":
                structRow.gender = Int(tagContent)!
            case "picture":
                    //TODO:需調整
                structRow.picture = nil
            case "phone":
