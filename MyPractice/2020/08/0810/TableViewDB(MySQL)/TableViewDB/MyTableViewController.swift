...
class MyTableViewController: UITableViewController, XMLParserDelegate
{
    ...
    //http://studio-pj.com/class_exercise/select_data.php
    //--------------------MySQL增加-------------------
    // 記錄主要網域名稱（網址開頭的部分）
...
//        }
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration?
    {
        ...
        let actionDelete = UIContextualAction(style: .normal, title: "刪除") { (action, view, completionHanlder) in
            ...
            //Step0-1.編成即將呼叫的網址
            let strURL = String(format: self.webDomain + "delete_data.php?no=%@", self.arrTable[indexPath.row].no);
            //Step0-2.以編碼過後的網址來初始化URL實體
            ...
            self.dataTask = self.session.dataTask(with: self.url, completionHandler: {
                ...
                if strEchoMessage == "1"
                {
                    DispatchQueue.main.async {
                        //製作訊息視窗
                        let alert = UIAlertController(title: "資料處理訊息", message: "資料刪除成功！", preferredStyle: .alert)
                        //初始化訊息視窗準備使用的按鈕
                        let btnOK = UIAlertAction(title: "確定", style: .default, handler: nil)
                        //將按鈕加入訊息視窗
                        alert.addAction(btnOK)
                ...
                else
                {
                    DispatchQueue.main.async {
                        //製作訊息視窗
                        let alert = UIAlertController(title: "資料處理訊息", message: "資料刪除失敗！", preferredStyle: .alert)
                        //初始化訊息視窗準備使用的按鈕
